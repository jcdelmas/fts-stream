...
export abstract class Chunk<A> implements Iterable<A> {
  ...
  drop(n: number): Chunk<A> {
    if (n === 0) return this
    else if (n >= this.size) return Chunk.empty()
    else return this._drop(n)
  }
  ...
  take(n: number): Chunk<A> {
    ...
    else if (n >= this.size) return this
    else return this._take(n)
  }
  ...
  splitAt(n: number): [Chunk<A>, Chunk<A>] {
    ...
    else if (n >= this.size) return [this, Chunk.empty()]
    else return [this._take(n), this._drop(n)]
  }
...
class Concat<A, B> extends Chunk<A | B> {
  ...
  protected _drop(n: number): Chunk<A | B> {
    ...
    else if (n < this.left.size) return this.left.drop(n).concat(this.right)
    else return this.right.drop(n - this.left.size)
  }

  protected _take(n: number): Chunk<A | B> {
    if (n === this.left.size) return this.left
    else if (n < this.left.size) return this.left.take(n)
    else return this.left.concat(this.right.take(n - this.left.size))
  }

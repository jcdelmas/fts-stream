...
describe('scan', () => {
  withFlowProp(<A>(init: any, f: (b: any, a: A) => any) => [(s: Stream<A>) => s.scan(init, f), (a: A[]) => scan(a, init, f)])
    .testStreams(0, (s: number, a: number) => s + a)

  testStreamPurity(Stream.range(1, 5).scan<number>(0, (s, a) => s + a))
})

function scan<A, B>(array: A[], b: B, f: (b: B, a: A) => B) {
  function loop(i: number, b: B): B[] {
    return [b, ...(i < array.length ? loop(i + 1, f(b, array[i])) : [])]
  }
  return loop(0, b)
}
...
function mapAccum<A, B, S>(array: A[], s: S, f: (s: S, a: A) => [S, B]) {

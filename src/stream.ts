...
export abstract class Stream<A> {
  ...
  scan<S>(s: S, f: (s: S, a: A) => S): Stream<S> {
    return Stream.single(s).concat(this.mapAccum(s, (s, a) => {
      const s2 = f(s, a)
      return [s2, s2]
    }))
  }

  mapAccum<B, S>(s: S, f: (s: S, a: A) => [S, B]): Stream<B> {
    return this.simplePipe(push => {
      let state = s
      return async chunk => {
  ...
  zipAllWith<B, C>(stream: Stream<B>, f: (a: A | undefined, b: B | undefined) => C | undefined): Stream<C> {
    return Stream.createSimple<C>(push => {
      async function takeToOption<A>(take: Promise<Take<A>>): Promise<A | undefined> {
  ...
  debounce(duration: number): Stream<A> {
    return Stream.createSimpleConcurrent(queue => {
      ...
      return this.foreach0(async a => {
        ...
        if (lastTimer) return new Promise<void>(resolve => lastCallback = resolve)
      })
...
export namespace Stream {
  ...
  export function merge<A, B, C, D, E>(s1: Stream<A>, s2: Stream<B>, s3: Stream<C>, s4: Stream<D>, s5: Stream<E>): Stream<A | B | C | D | E>

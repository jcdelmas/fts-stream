...
describe('map', () => {
  ...
  testStreamPurity(Stream.range(1, 6).map(n => n + 1))
})

describe('flatMap', () => {
  withFlowProp(() => [
    (s: Stream<number>) => s.flatMap((a: number) => Stream.range(a, a + 6)),
    (as: number[]) => _.flatten(as.map(a => _.range(a, a + 6, 1))),
  ]).testStreams()

  testStreamPurity(Stream.range(1, 6).flatMap(n => Stream.single(n + 1)))

  // Laws
  const f: (n: number) => Stream<number> = n => Stream.range(n, n + 6)
...
describe('mapConcat', () => {
  ...
  testStreamPurity(Stream.range(1, 6).mapConcat((a: number) => [a, a + 2, a - 3]))
})
...
describe('take', () => {
  ...
  testStreamPurity(Stream.range(1, 6).take(3))
})
...
describe('filter', () => {
  ...
  testStreamPurity(Stream.range(1, 6).filter(n => n < 3))
})
...
describe('takeWhile', () => {
  ...
  testStreamPurity(Stream.range(1, 6).takeWhile(n => n < 3))
})
...
describe('drop', () => {
  ...
  testStreamPurity(Stream.range(1, 6).drop(2))
})
...
describe('dropWhile', () => {
  ...
  testStreamPurity(Stream.range(1, 6).dropWhile(n => n < 3))
})
...
describe('scan', () => {
  ...
  testStreamPurity(Stream.range(1, 6).scan<number>(0, (s, a) => s + a))
})
...
describe('mapAccum', () => {
  ...
  testStreamPurity(Stream.range(1, 6).mapAccum<number, number>(0, (s, a) => [s + a, s]))
})
...
describe('intersperse', () => {
  ...
  testStreamPurity(Stream.range(1, 6).intersperse('_'))
})
...
describe('mapAsync', () => {
  ...
  testStreamPurity(Stream.range(1, 6).mapAsync(n => Promise.resolve(n + 1)))
...
describe('mapAsyncUnordered', () => {
  ...
  testStreamPurity(Stream.range(1, 6).mapAsyncUnordered(n => Promise.resolve(n + 1), 1))
...
describe('concat', () => {
  ...
  testStreamPurity(Stream.range(1, 4).concat(Stream.from([4, 5])))

  test('with early stop', async () => {
    const result = await Stream.range(1, 4)
      .concat(Stream.range(4, 6))
      .take(2)
      .toArray()
    expect(result).toEqual([1, 2])
  })
...
describe('zipAllWith', () => {
  ...
  test('right shorter', async () => {
    const result = await Stream.from(['foo', 'bar'])
      .zipAllWith(Stream.range(1, 4), (a, b) => (a || '') + (b || ''))
      .toArray()
...
describe('zipWith', () => {
  ...
  test('left shorter', async () => {
    const result = await Stream.range(1, 4)
      .zipWith(Stream.range(4, 8), (a, b) => a + b)
      .toArray()
    expect(result).toEqual([5, 7, 9])
  })
  test('right shorter', async () => {
  ...
  test('left error', async () => {
    const result = await Stream.single(1)
      .concat(Stream.failed<number>('my error'))
      .zipWith(Stream.range(2, 5), (a, b) => a + b)
      .recover(err => err)
      .toArray()
  ...
  testStreamPurity(Stream.range(1, 4).merge(Stream.range(11, 14)))
})
